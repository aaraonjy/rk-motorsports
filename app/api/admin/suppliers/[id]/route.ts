...
export async function DELETE(
  ...
  if (!supplier) {
    return Response.json({ ok: false, error: "Supplier not found." }, { status: 404 });
  }

  const totalLinkedRecords = 0;

  if (totalLinkedRecords > 0) {
    return Response.json(
      {
        ok: false,
        error: "This supplier already has transactions and cannot be deleted. Please set the supplier as inactive instead.",
      },
      { status: 409 }
    );
  }
