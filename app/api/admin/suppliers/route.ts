...
import { resolveSupplierAccountNo } from "@/lib/supplier-account";
...
export async function POST(req: Request) {
  ...
  if (phone) {
    ...
    if (existingPhone) return Response.json({ ok: false, error: "This phone number is already used by another supplier." }, { status: 409 });
  }
  ...
  const accountNoResult = await resolveSupplierAccountNo({
    db,
    supplierName: name,
    overrideAccountNo: getNullableText(body, "supplierAccountNo"),
  });
  ...
  await db.supplier.create({
    data: {
      ...
      supplierAccountNo: accountNoResult.supplierAccountNo,
