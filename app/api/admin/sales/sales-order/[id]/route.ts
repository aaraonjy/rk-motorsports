...
function getBaseRevisionDocNo(docNo: string | null | undefined) {
  ...
  if (!value) throw new Error("Current Sales Order No is invalid for revision.");

  const match = value.match(/^(SO-\d{8}-\d{4})(?:-(\d+))?$/);
  if (!match) throw new Error("Current Sales Order No is invalid for revision.");
...
async function buildSalesOrderData(body: any) {
...
export async function GET(_req: Request, { params }: Params) {
  try {
    ...
    if (!transaction || transaction.docType !== "SO") {
      return NextResponse.json({ ok: false, error: "Sales Order not found." }, { status: 404 });
    }
    ...
    return NextResponse.json({ ok: true, transaction: transactionWithProgress });
  } catch (error) {
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Unable to load sales order." },
  ...
  try {
    ...
    if (action === "cancel") {
      ...
      const updated = await db.$transaction(async (tx) => {
        ...
        const sourceLinks = await tx.salesTransactionLink.findMany({
          where: { targetTransactionId: id },
          select: { sourceTransactionId: true },
        });

        for (const link of sourceLinks) {
          const hasOtherActiveTargets = await tx.salesTransactionLink.findFirst({
            where: {
              sourceTransactionId: link.sourceTransactionId,
              targetTransactionId: { not: id },
              targetTransaction: { status: { not: "CANCELLED" } },
            },
            select: { id: true },
          });
          ...
          if (!hasOtherActiveTargets) {
            await tx.salesTransaction.updateMany({
              where: { id: link.sourceTransactionId, docType: "QO", status: "CONFIRMED" },
              data: { status: "PENDING" },
            });
          }
        }
      ...
      await createAuditLogFromRequest({
        ...
        description: `${admin.name} cancelled sales order ${updated.docNo}.`,
    ...
    const prepared = await buildSalesOrderData(body);
    ...
    await createAuditLogFromRequest({
      ...
      description: action === "revise" ? `${admin.name} revised sales order ${saved.docNo}.` : `${admin.name} edited sales order ${saved.docNo}.`,
      newValues: {
        docNo: saved.docNo,
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : "Unable to update sales order." },
