    ...
    if (action === "cancel") {
      ...
      if (activeDownstream?.targetTransaction) {
        return NextResponse.json(
          {
            ok: false,
            error: `Cannot cancel this Quotation because it has an active downstream ${activeDownstream.targetTransaction.docType} (${activeDownstream.targetTransaction.docNo}). Please cancel the downstream document first.`,
          },
          { status: 400 }
        );
      }
