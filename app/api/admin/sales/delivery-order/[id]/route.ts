...
async function reverseStockIssueForDeliveryOrder(tx: Prisma.TransactionClient, deliveryOrder: any, adminId: string, cancelReason: string | null) {
  ...
  if (ledgerEntries.length === 0) {
    await reverseLegacyStockIssueForDeliveryOrder(tx, deliveryOrder, adminId, cancelReason);
    return;
  }
  ...
  for (const entry of activeLedgerEntries) {
    ...
    await tx.stockLedger.create({
      data: {
        ...
        sourceType: "SALES_DELIVERY_ORDER_CANCEL",
        sourceId: deliveryOrder.id,
        remarks: [cancelReason || `Cancellation reversal for ${deliveryOrder.docNo}`, `REVERSAL_OF_LEDGER_ID=${entry.id}`].filter(Boolean).join(" | "),
      },
    });
...
async function reverseLegacyStockIssueForDeliveryOrder(tx: Prisma.TransactionClient, deliveryOrder: any, adminId: string, cancelReason: string | null) {
