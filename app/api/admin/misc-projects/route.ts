...
export async function POST(req: Request) {
  try {
    ...
    const created = await db.project.create({ data: { code, name, isActive } });
    return NextResponse.json({ ok: true, item: mapItem(created) });
  } catch (error) {
    return NextResponse.json({ ok: false, error: error instanceof Error ? error.message : "Unable to create project." }, { status: 500 });
  }
}
