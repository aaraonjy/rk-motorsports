...
export async function PATCH(req: Request, { params }: Params) {
  try {
    ...
    const transaction = await updatePurchaseTransaction("PI" as any, id, body, admin.id);
...
export async function DELETE(req: Request, { params }: Params) {
  try {
    ...
    const transaction = await cancelPurchaseTransaction("PI" as any, id, admin.id, reason);
    return NextResponse.json({ ok: true, transaction });
  } catch (error) {
