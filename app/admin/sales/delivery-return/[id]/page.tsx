...
function sourceTransactionHref(source?: { id?: string | null; docType?: string | null } | null) {
  if (!source?.id) return "#";
  if (source.docType === "DO") return `/admin/sales/delivery-order/${source.id}`;
  if (source.docType === "SO") return `/admin/sales/sales-order/${source.id}`;
  if (source.docType === "INV") return `/admin/sales/sales-invoice/${source.id}`;
  if (source.docType === "CN") return `/admin/sales/credit-note/${source.id}`;
  if (source.docType === "CS") return `/admin/sales/cash-sales/${source.id}`;
  if (source.docType === "DR") return `/admin/sales/delivery-return/${source.id}`;
  return "#";
}

function targetTransactionHref(target?: { id?: string | null; docType?: string | null } | null) {
  return sourceTransactionHref(target);
}

function getLineSourceDocNos(line: {
  targetLineLinks?: Array<{ sourceTransaction?: { docNo?: string | null } | null }>;
  sourceLineLinks?: Array<{ sourceTransaction?: { docNo?: string | null } | null }>;
}) {
  const fromTargetLinks = (line.targetLineLinks || []).map((link) => link.sourceTransaction?.docNo).filter(Boolean) as string[];
  const fromSourceLinks = (line.sourceLineLinks || []).map((link) => link.sourceTransaction?.docNo).filter(Boolean) as string[];
  return uniqueText([...fromTargetLinks, ...fromSourceLinks]).join(", ") || "-";
}
...
export default async function AdminDeliveryReturnDetailPage({ params }: Params) {
    ...
    return (
      <section className="section-pad">
        <div className="container-rk max-w-5xl">
          <p className="text-white/70">Delivery Return not found.</p>
        </div>
      </section>
    );
  }
  ...
  const stockLedgerRows = await db.stockLedger.findMany({
    where: { sourceType: "DELIVERY_RETURN", sourceId: transaction.id, movementDirection: "IN" },
      ...
      <div className="container-rk max-w-7xl space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-white/70">View delivery return details in read-only mode.</p>
            {generatedFrom.length > 0 ? (
              <div className="mt-3 flex flex-wrap items-center gap-2 text-sm">
                <span className="text-sky-200">Generated from:</span>
                {generatedFrom.map((source, index) => (
                  <Link key={source?.id} href={sourceTransactionHref(source)} className="text-sky-200 underline-offset-4 hover:underline">
            ...
            {generatedTo.length > 0 ? (
              <div className="mt-2 flex flex-wrap items-center gap-2 text-sm">
                ...
                {generatedTo.map((target, index) => (
                  <Link key={target?.id} href={targetTransactionHref(target)} className="text-sky-200 underline-offset-4 hover:underline">
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/admin/sales/delivery-return" className="rounded-xl border border-white/15 bg-white/5 px-5 py-3 text-sm text-white/80 transition hover:bg-white/10">
              Back
            </Link>
          </div>
        </div>
        ...
        {transaction.status === "CANCELLED" ? (
          <div className="rounded-2xl border border-red-500/25 bg-red-500/10 p-5 text-sm text-red-100">
            <div className="font-semibold">This delivery return has been cancelled.</div>
        ...
        <div className="rounded-[2rem] border border-white/10 bg-black/45 p-5 backdrop-blur-md md:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-white/40">Delivery Return</p>
              <h2 className="mt-4 text-4xl font-bold">View Delivery Return</h2>
              <p className="mt-4 max-w-3xl text-white/70">Delivery Return performs stock in immediately when created.</p>
            </div>
                ...
                <tbody className="divide-y divide-white/10 text-white/80">
                  {transaction.lines.length === 0 ? (
                    ...
                    transaction.lines.map((line) => {
                      const matchingLedgerRows = stockLedgerRows.filter((row) =>
                        row.inventoryProductId === line.inventoryProductId && (!line.locationId || row.locationId === line.locationId)
                      );
                      const fallbackLedgerRows = matchingLedgerRows.length > 0 ? matchingLedgerRows : stockLedgerRows.filter((row) => row.inventoryProductId === line.inventoryProductId);
                      ...
                      return (
                        <tr key={line.id}>
                          ...
                          <td className="px-4 py-4">{getLineSourceDocNos(line)}</td>
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
            ...
            <div className="rounded-[1.5rem] border border-white/10 p-5">
              <h3 className="text-xl font-bold">Delivery Return Summary</h3>
