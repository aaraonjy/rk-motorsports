...
export default async function AdminDebitNoteDetailPage({ params }: Params) {
    ...
    return (
      <section className="section-pad">
        <div className="container-rk max-w-5xl">
          <p className="text-white/70">Debit Note not found.</p>
        </div>
      </section>
    );
  }
  ...
  const stockLedgerRows = await db.stockLedger.findMany({
    where: { sourceType: "DEBIT_NOTE", sourceId: transaction.id, movementDirection: "OUT" },
      ...
      <div className="container-rk max-w-7xl space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-white/70">View debit note details in read-only mode.</p>
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/admin/sales/debit-note" className="rounded-xl border border-white/15 bg-white/5 px-5 py-3 text-sm text-white/80 transition hover:bg-white/10">
              Back
            </Link>
          </div>
        </div>
        ...
        {transaction.status === "CANCELLED" ? (
          <div className="rounded-2xl border border-red-500/25 bg-red-500/10 p-5 text-sm text-red-100">
            <div className="font-semibold">This debit note has been cancelled.</div>
        ...
        <div className="rounded-[2rem] border border-white/10 bg-black/45 p-5 backdrop-blur-md md:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-white/40">Debit Note</p>
              <h2 className="mt-4 text-4xl font-bold">View Debit Note</h2>
              <p className="mt-4 max-w-3xl text-white/70">Debit Note increases sales amount and stocks stock items out from inventory.</p>
            </div>
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
            ...
            <div className="rounded-[1.5rem] border border-white/10 p-5">
              <h3 className="text-xl font-bold">Debit Note Summary</h3>
