...
export default async function AdminSalesInvoiceDetailPage({ params }: Params) {
    ...
    return (
      <section className="section-pad">
        <div className="container-rk max-w-5xl">
          <p className="text-white/70">Cash Sales not found.</p>
        </div>
      </section>
    );
  }
  ...
  const stockLedgerRows = await db.stockLedger.findMany({
    where: { sourceType: "SALES_CASH_SALES", sourceId: transaction.id, movementDirection: "OUT" },
    orderBy: [{ createdAt: "asc" }],
      ...
      <div className="container-rk max-w-7xl space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-white/70">View cash sales details in read-only mode.</p>
          ...
          <div className="flex flex-wrap gap-3">
            <Link href="/admin/sales/cash-sales" className="rounded-xl border border-white/15 bg-white/5 px-5 py-3 text-sm text-white/80 transition hover:bg-white/10">
              Back
            </Link>
            <Link
              href={`/admin/sales/cash-sales?edit=${transaction.id}`}
        ...
        <div className="rounded-[2rem] border border-white/10 bg-black/45 p-5 backdrop-blur-md md:p-6">
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-white/40">Cash Sales</p>
              <h2 className="mt-4 text-4xl font-bold">View Cash Sales</h2>
              <p className="mt-4 max-w-3xl text-white/70">Cash Sales performs stock out immediately when created.</p>
            </div>
          ...
          <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_360px]">
            ...
            <div className="rounded-[1.5rem] border border-white/10 p-5">
              <h3 className="text-xl font-bold">Cash Sales Summary</h3>
