...
export default async function AdminPOPage() {
  ...
  const data = await loadSharedData("PO" as PurchaseDocType);
  return <section className="section-pad"><div className="container-rk"><h1 className="text-4xl font-bold">Purchase Order</h1><p className="mt-4 text-white/70">Create and manage purchase orders. No stock or purchase amount is posted from PO.</p><div className="mt-8"><AdminPurchaseTransactionClient docType="PO" title="Purchase Order" description="Create and manage purchase orders. No stock or purchase amount is posted from PO." apiPath="/api/admin/purchase/purchase-order" initialTransactions={data.transactions as any} sourceDocuments={data.sourceDocuments as any} suppliers={data.suppliers as any} products={data.products as any} locations={data.locations} agents={data.agents} projects={data.projects} departments={data.departments} defaultLocationId={data.defaultLocationId} projectFeatureEnabled={data.projectFeatureEnabled} departmentFeatureEnabled={data.departmentFeatureEnabled} taxConfig={data.taxConfig} /></div></div></section>;
}
