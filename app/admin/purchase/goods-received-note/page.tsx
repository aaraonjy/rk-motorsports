...
export default async function AdminGRNPage() {
  const user = await getSessionUser();
  if (!user) redirect("/login");
  if (user.role !== "ADMIN") redirect("/dashboard");
  const data = await loadSharedData("GRN" as PurchaseDocType);
  return <section className="section-pad"><div className="container-rk"><h1 className="text-4xl font-bold">Goods Received Note</h1><p className="mt-4 text-white/70">Receive stock from supplier, with optional generation from Purchase Order.</p><div className="mt-8"><AdminPurchaseTransactionClient docType="GRN" title="Goods Received Note" description="Receive stock from supplier, with optional generation from Purchase Order." apiPath="/api/admin/purchase/goods-received-note" initialTransactions={data.transactions as any} sourceDocuments={data.sourceDocuments as any} suppliers={data.suppliers as any} products={data.products as any} locations={data.locations} agents={data.agents} projects={data.projects} departments={data.departments} defaultLocationId={data.defaultLocationId} projectFeatureEnabled={data.projectFeatureEnabled} departmentFeatureEnabled={data.departmentFeatureEnabled} taxConfig={data.taxConfig} /></div></div></section>;
}
