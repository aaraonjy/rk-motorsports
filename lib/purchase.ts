import { Prisma, PurchaseDocType, PurchaseLineLinkType, PurchaseTransactionStatus, StockMovementDirection, StockTransactionStatus, StockTransactionType } from "@prisma/client";
import { db } from "@/lib/db";
import { acquireAdvisoryLock, acquireStockMutationLocks, buildLedgerValues, createStoredQtyDecimal, generateStockTransactionNumber } from "@/lib/stock";
import {
...
function decimal(value: number | string | null | undefined, places = 2, fallback = 0) {
...
async function generatePurchaseDocNo(tx: Prisma.TransactionClient, docType: PurchaseDocType, date: Date) {
...
async function loadTaxSettings(tx: Prisma.TransactionClient) {
  ...
  return {
    ...
    taxCalculationMode: normalizeTaxCalculationMode(config?.taxCalculationMode) as TaxCalculationModeValue,
    defaultAdminTaxCodeId: config?.defaultAdminTaxCodeId || null,
  };
}

async function snapshotTaxCode(tx: Prisma.TransactionClient, taxCodeId?: string | null) {
...
async function mapLine(tx: Prisma.TransactionClient, line: PurchaseLinePayload, index: number, taxEnabled: boolean, defaultTaxCodeId: string | null) {
  ...
  const product = inventoryProductId
    ? await tx.inventoryProduct.findUnique({
        ...
        select: { id: true, code: true, description: true, baseUom: true, itemType: true, unitCost: true },
      })
    : null;
  ...
  if (!productCode || !productDescription) throw new Error(`Line ${index + 1}: Product is required.`);
  ...
  const discountAmountNumber = discountType === "AMOUNT" ? toNumber(discountRate) : lineSubtotalNumber * (toNumber(discountRate) / 100);
  const lineTotalNumber = Math.max(0, Math.round((lineSubtotalNumber - discountAmountNumber + Number.EPSILON) * 100) / 100);
  const taxCode = await snapshotTaxCode(tx, normalizeText(line.taxCodeId) || defaultTaxCodeId);
  ...
  const location = normalizeText(line.locationId)
    ? await tx.stockLocation.findUnique({ where: { id: normalizeText(line.locationId)! }, select: { id: true, code: true, name: true } })
    : null;
...
async function mapLines(tx: Prisma.TransactionClient, lines: PurchaseLinePayload[] | undefined, taxEnabled: boolean, defaultTaxCodeId: string | null) {
  const rawLines = Array.isArray(lines) ? lines : [];
  if (rawLines.length === 0) throw new Error("At least one item line is required.");
  return Promise.all(rawLines.map((line, index) => mapLine(tx, line, index, taxEnabled, defaultTaxCodeId)));
}

function calculateTotals(lines: Awaited<ReturnType<typeof mapLines>>, taxMode: TaxCalculationModeValue, taxEnabled: boolean, taxCode: any) {
  const subtotal = lines.reduce((sum, line) => sum + toNumber(line.lineSubtotal), 0);
  const discountTotal = lines.reduce((sum, line) => sum + toNumber(line.discountAmount), 0);
  if (taxMode === "LINE_ITEM") {
    const taxTotal = lines.reduce((sum, line) => sum + toNumber(line.taxAmount), 0);
    const grandTotal = lines.reduce((sum, line) => sum + toNumber(line.lineTotal), 0);
    return { subtotal, discountTotal, taxableSubtotal: subtotal - discountTotal, taxTotal, grandTotal };
  }
...
async function assertActiveSupplier(tx: Prisma.TransactionClient, supplierId: string) {
  const supplier = await tx.supplier.findUnique({ where: { id: supplierId } });
  if (!supplier || !supplier.isActive) throw new Error("Selected supplier is inactive or unavailable.");
  return supplier;
}

async function createStockReceive(tx: Prisma.TransactionClient, transaction: any, lines: any[]) {
  const stockLines = lines.filter((line) => line.inventoryProductId && line.locationId && line.itemType === "STOCK_ITEM");
  if (stockLines.length === 0) return null;
  await acquireStockMutationLocks(tx, stockLines.map((line) => ({ inventoryProductId: line.inventoryProductId!, locationId: line.locationId!, batchNo: line.batchNo })));
  const stockTransactionNo = await generateStockTransactionNumber(tx, StockTransactionType.SR, transaction.docDate);
  ...
  for (const stockLine of stockTransaction.lines) {
    const values = buildLedgerValues(createStoredQtyDecimal(stockLine.qty), "IN");
    ...
    if (stockLine.unitCost) {
      await tx.inventoryProduct.update({ where: { id: stockLine.inventoryProductId }, data: { unitCost: stockLine.unitCost } });
    }
  }
  await tx.purchaseTransaction.update({ where: { id: transaction.id }, data: { stockTransactionId: stockTransaction.id } });
...
async function refreshSourceStatus(tx: Prisma.TransactionClient, sourceTransactionId: string) {
  const source = await tx.purchaseTransaction.findUnique({
    ...
    include: { lines: { include: { sourceLineLinks: { include: { targetTransaction: true } } } } },
  });
  if (!source || source.status === "CANCELLED") return;
  if (source.lines.length === 0) return;
  const linkType: PurchaseLineLinkType = source.docType === "GRN" ? "INVOICED_TO" : "RECEIVED_TO";
  ...
  for (const line of source.lines) {
    const fulfilled = line.sourceLineLinks
      ...
      .filter((link) => link.linkType === linkType || (source.docType === "PO" && link.linkType === altLinkType))
      .reduce((sum, link) => sum + toNumber(link.qty), 0);
    if (fulfilled > 0) any = true;
    if (fulfilled + 0.0001 < toNumber(line.qty)) all = false;
  }
  const status: PurchaseTransactionStatus = all ? "COMPLETED" : any ? "PARTIAL" : "OPEN";
  await tx.purchaseTransaction.update({ where: { id: source.id }, data: { status } });
}

function shouldPostStock(docType: PurchaseDocType, sourceDocType?: PurchaseDocType | null) {
...
export async function createPurchaseTransaction(docType: PurchaseDocType, body: PurchasePayload, adminId: string) {
  return db.$transaction(async (tx) => {
    ...
    const manualDocNo = assertValidManualDocNo(docType, body.docNo);
    const docNo = manualDocNo || (await generatePurchaseDocNo(tx, docType, docDate));
...
export async function loadPurchaseSources(docType: PurchaseDocType) {
  ...
  return sources.map((source) => ({
    ...
    lines: source.lines.map((line) => {
      const linkType: PurchaseLineLinkType = source.docType === "GRN" ? "INVOICED_TO" : docType === "GRN" ? "RECEIVED_TO" : "INVOICED_TO";
