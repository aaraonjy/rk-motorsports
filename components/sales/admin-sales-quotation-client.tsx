...
function buildQuotationRevisionDocNoPreview(transaction: QuotationRecord) {
  const baseDocNo = getBaseQuotationDocNo(transaction.revisedFrom?.docNo || transaction.docNo);
  let maxRevision = 0;
...
export function AdminSalesQuotationClient({
  ...
  async function loadNextQuotationDocNoPreview(nextDocDate = docDate) {
    const fallbackDocNo = buildQuotationDocNoPreview(nextDocDate, transactions);
  ...
  function resetForm() {
    ...
    setAutoGeneratedDocNoPreview(buildQuotationDocNoPreview(nextDocDate, transactions));
    setDocNo("");
    setDocNoDraft("");
    setIsDocNoModalOpen(false);
    setDocDesc("");
  ...
  function fillFormFromTransaction(transaction: QuotationRecord, mode: "edit" | "revise") {
    setFormMode(mode);
    ...
    setAutoGeneratedDocNoPreview(mode === "revise" ? buildQuotationRevisionDocNoPreview(transaction) : transaction.docNo);
    setIsDocNoModalOpen(false);
  ...
  async function openCreate() {
    ...
    await loadNextQuotationDocNoPreview(todayInput());
  }

  function openEdit(transaction: QuotationRecord) {
  ...
  async function submitQuotation() {
    ...
    try {
      ...
      const endpoint = isUpdateMode && editTarget ? `/api/admin/sales/quotation/${editTarget.id}` : "/api/admin/sales/quotation";
    ...
    setRecentCancelledTransaction(null);
      ...
      setSubmitError(error instanceof Error ? error.message : "Unable to save quotation.");
    } finally {
        ...
        <div className="fixed inset-0 z-[120] overflow-y-auto bg-black/75 px-4 py-8 backdrop-blur-sm">
          <div className="mx-auto max-w-7xl rounded-[2rem] border border-white/10 bg-[#08080c] p-6 shadow-2xl">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.24em] text-red-400/80">Sales Quotation</p>
                <h2 className="mt-3 text-3xl font-bold">{formMode === "revise" ? "Revise Quotation" : formMode === "edit" ? "Edit Quotation" : "Create Quotation"}</h2>
            ...
            <div className="mt-8 flex flex-wrap justify-end gap-3 border-t border-white/10 pt-5">
              ...
              <button type="button" onClick={submitQuotation} disabled={isSubmitting} className="rounded-xl bg-red-500 px-5 py-3 text-sm font-semibold text-white transition hover:bg-red-400 disabled:cursor-not-allowed disabled:opacity-60">
                {isSubmitting ? "Saving..." : formMode === "revise" ? "Save Revised Quotation" : formMode === "edit" ? "Update Quotation" : "Save Quotation"}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/75 px-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-[2rem] border border-white/10 bg-[#08080c] p-6 shadow-2xl">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button type="button" onClick={cancelQuotation} className="rounded-xl bg-red-500 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-red-400">Confirm Cancel</button>
            </div>
          </div>
        </div>
      ) : null}
