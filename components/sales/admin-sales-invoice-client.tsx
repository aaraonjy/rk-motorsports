...
export function AdminSalesInvoiceClient({
  ...
  const selectedCustomer = useMemo(() => initialCustomers.find((customer) => customer.id === customerId) || null, [customerId, initialCustomers]);
                      ...
                      <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                        ...
                        {selectedProduct?.batchTracking ? (
                          <div className="md:col-span-2">
                            ...
                            />
                            {line.inventoryProductId && line.locationId && !line.batchNo && !isGeneratedLine && !loadingBatches[index] && (availableBatches[index] || []).length === 0 ? (
        ...
        <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-[2rem] border border-white/10 bg-[#0b0b0f] p-6 shadow-2xl">
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button type="button" onClick={() => { setDocNo(normalizeDocNoInput(docNoDraft)); setIsDocNoModalOpen(false); }} className="rounded-xl bg-red-500 px-5 py-3 font-semibold text-white transition hover:bg-red-400">OK</button>
            </div>
          </div>
        </div>
      ) : null}
