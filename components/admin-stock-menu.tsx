    ...
    <div ref={dropdownRef} className="relative">
      ...
      {isOpen ? (
        <div className="absolute left-0 top-[calc(100%+12px)] z-[70] min-w-[250px] overflow-hidden rounded-2xl border border-white/10 bg-[#0b0b0c]/95 shadow-2xl backdrop-blur-xl">
          ...
          {activeItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`block px-4 py-4 text-sm transition hover:bg-white/10 hover:text-white ${
          ...
          <div className="border-t border-white/10 px-4 py-3 text-[11px] font-semibold uppercase tracking-[0.24em] text-white/35">
            Coming Soon
          </div>
          ...
          <div className="grid gap-1 px-2 py-2">
            {upcomingItems.map((label) => (
              <div
                key={label}
                className="rounded-xl px-3 py-2 text-sm text-white/35"
              >
                {label}
              </div>
            ))}
