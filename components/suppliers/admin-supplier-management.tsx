...
type SupplierRecord = {
  ...
  _count: {
    orders: number;
    supplierSalesTransactions?: number;
    creditNotes?: number;
  };
  salesTransactionOrderCount?: number;
  supplierProfileTransactionCount?: number;
};
...
type Props = {
  ...
  existingSupplierAccountNos: string[];
  currentPage: number;
  pageSize: number;
};
...
type SupplierFormState = {
  name: string;
  supplierAccountNoSuffix: string;
  email: string;
  phone: string;
  phone2: string;
  fax: string;
...
function mapDeliveryAddress(address: SupplierDeliveryAddress): DeliveryAddressFormState {
  return {
    id: address.id,
...
function getInitialForm(supplier: SupplierRecord | null): SupplierFormState {
  const currentSuffix = supplier?.supplierAccountNo?.split("/")[1]?.slice(1) || "";

  return {
    name: supplier?.name || "",
    supplierAccountNoSuffix: currentSuffix,
    ...
    billingAddressLine3: supplier?.billingAddressLine3 || "",
    billingAddressLine4: supplier?.billingAddressLine4 || "",
    billingCity: supplier?.billingCity || "",
    billingPostCode: supplier?.billingPostCode || "",
    billingCountryCode: supplier?.billingCountryCode || "MY",
    deliveryAddressLine1: supplier?.deliveryAddressLine1 || "",
    deliveryAddressLine2: supplier?.deliveryAddressLine2 || "",
    deliveryAddressLine3: supplier?.deliveryAddressLine3 || "",
    deliveryAddressLine4: supplier?.deliveryAddressLine4 || "",
    deliveryCity: supplier?.deliveryCity || "",
    deliveryPostCode: supplier?.deliveryPostCode || "",
    deliveryCountryCode: supplier?.deliveryCountryCode || "MY",
    deliveryAddresses: (supplier?.deliveryAddresses || []).map(mapDeliveryAddress),
    area: supplier?.area || "",
    ...
    registrationIdType: supplier?.registrationIdType || "BRN",
    registrationNo: supplier?.registrationNo || "",
    taxIdentificationNo: supplier?.taxIdentificationNo || "",
    creditControlType: Number(supplier?.creditTermsDays ?? 0) > 0 ? "TERMS" : Number(supplier?.creditLimitAmount ?? 0) > 0 ? "LIMIT" : "NONE",
    creditTermsDays: String(supplier?.creditTermsDays ?? 0),
    creditLimitAmount: String(supplier?.creditLimitAmount ?? 0),
...
function getNextAccountSuffix(args: {
  ...
  const nextSequence =
    args.existingSupplierAccountNos.reduce((max, accountNo) => {
...
function getSupplierStatusLabel(isActive: boolean) {
  return isActive ? "Active" : "Inactive";
}

function getCreditBadge(supplier: SupplierRecord) {
  if (supplier.creditOverdue) {
  ...
  if (supplier.creditLimitExceeded) {
...
function getCreditLabel(supplier: SupplierRecord) {
  if (supplier.creditOverdue) return "Overdue";
  if (supplier.creditLimitExceeded) return "Over Limit";
  return "OK";
}
...
function SupplierModal({
  ...
  const [form, setForm] = useState<SupplierFormState>(() => getInitialForm(supplier));
  ...
  useMemo(() => {
    setForm(getInitialForm(supplier));
  ...
  const accountSuffixLength = getAccountSuffixLength(accountConfiguration.supplierAccountNoFormat);
  const sequenceDigits = getSequenceDigits(accountConfiguration.supplierAccountNoFormat);
  const accountPrefix = getAccountPrefix(accountConfiguration.supplierAccountPrefix);
  const supplierInitial = form.name.trim() ? getSupplierInitial(form.name) : "";
  const defaultAccountSuffix = useMemo(
    ...
    [accountPrefix, supplierInitial, existingSupplierAccountNos, sequenceDigits]
  );
  const rawAccountSuffix = accountSuffixTouched ? form.supplierAccountNoSuffix : defaultAccountSuffix;
  const accountSuffix = supplierInitial && rawAccountSuffix ? rawAccountSuffix.toUpperCase().slice(0, accountSuffixLength) : "";
  const previewAccountNo = supplierInitial && accountSuffix ? `${accountPrefix}/${accountSuffix}` : "";
  ...
  function updateField<K extends keyof SupplierFormState>(key: K, value: SupplierFormState[K]) {
    setForm((prev) => {
      const next = {
        ...prev,
        [key]: value,
        ...(key === "name" && mode === "create" ? { supplierAccountNoSuffix: "" } : {}),
      };
  ...
  function openAccountOverrideModal() {
    if (mode !== "create" || !supplierInitial) return;
  ...
  function saveAccountOverride() {
    ...
    updateField("supplierAccountNoSuffix", value);
    setIsAccountOverrideOpen(false);
  }
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(mode === "create" ? "/api/admin/suppliers" : `/api/admin/suppliers/${supplier?.id}`, {
        ...
        body: JSON.stringify({
          ...
          name: form.name.trim(),
          supplierAccountNo: mode === "create" ? previewAccountNo || null : supplier?.supplierAccountNo,
      ...
      if (!response.ok || !data.ok) {
        setError(data.error || "Unable to save supplier right now.");
        return;
      }

      onSaved(mode === "create" ? "Supplier created successfully." : "Supplier updated successfully.");
      onClose();
    } catch {
      setError("Unable to save supplier right now.");
    } finally {
      setIsSubmitting(false);
    }
  }
  ...
  function handleDeleteSupplier() {
    if (!supplier || !canDeleteSupplier) return;
  ...
  async function confirmDeleteSupplier() {
    ...
    try {
      ...
      if (!response.ok || !data.ok) {
        setError(data.error || "Unable to delete supplier right now.");
        setIsDeleteConfirmOpen(false);
        return;
      }

      onSaved("Supplier deleted successfully.");
      setIsDeleteConfirmOpen(false);
      onClose();
    } catch {
      setError("Unable to delete supplier right now.");
    } finally {
      setIsDeleting(false);
    }
  }
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/70 px-4 py-6">
      <div className="max-h-[90vh] w-full max-w-5xl overflow-y-auto rounded-2xl border border-white/10 bg-zinc-950 p-6 shadow-2xl">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-white">{mode === "create" ? "Add Supplier" : "Edit Supplier"}</h3>
            <p className="mt-1 text-sm text-white/50">
              {mode === "create" ? "Create a full supplier profile for purchase and payable records." : "Update the supplier profile details below."}
            </p>
          </div>
                ...
                {mode === "create" ? (
                  <button
                    ...
                    className={`flex min-h-[46px] w-full rounded-xl border border-white/10 bg-black/40 text-left text-sm text-white transition hover:border-white/20 ${
                      supplierInitial ? "cursor-pointer" : "cursor-default"
                    }`}
                    title={supplierInitial ? "Click to override A/C No suffix" : ""}
                  >
                  ...
                  <div className="rounded-xl border border-white/10 bg-black/40 px-4 py-3 text-sm text-white/70">{supplier?.supplierAccountNo || "-"}</div>
                )}
              </div>
              <div>
                <FieldLabel>Supplier Name <span className="text-red-300">*</span></FieldLabel>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
              ...
              <div className="mt-4 space-y-4">
                {[1, 2, 3, 4].map((line) => {
                  const key = `billingAddressLine${line}` as keyof SupplierFormState;
            ...
            <div className="rounded-2xl border border-white/10 bg-black/20 p-5">
              ...
              <div className="mt-4 space-y-4">
                {[1, 2, 3, 4].map((line) => {
                  const key = `deliveryAddressLine${line}` as keyof SupplierFormState;
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              ...
              <div>
                <SearchableSelect
                  ...
                  onChange={(option) => updateField("creditControlType", (option?.id as SupplierFormState["creditControlType"]) || "NONE")}
                />
                <p className="mt-2 text-xs text-white/40">Only one credit control method can be active per supplier.</p>
              </div>
          ...
          <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex items-center justify-end gap-3">
              ...
              <button type="submit" disabled={isSubmitting || isDeleting} className="rounded-xl border border-white/15 bg-black/30 px-4 py-2.5 text-sm text-white transition hover:bg-white/10 disabled:cursor-not-allowed disabled:opacity-50">
                {isSubmitting ? (mode === "create" ? "Creating..." : "Saving...") : mode === "create" ? "Add Supplier" : "Save Changes"}
              </button>
            </div>
          </div>
        </form>
        ...
        {isAccountOverrideOpen ? (
          <div className="fixed inset-0 z-[130] flex items-center justify-center bg-black/75 px-4">
            <div className="w-full max-w-lg rounded-[2rem] border border-white/10 bg-zinc-950 p-6 shadow-2xl">
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-white/45">Supplier A/C No</p>
              ...
              <div className="mt-4 grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)] gap-3">
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder={`${supplierInitial || "A"}${"0".repeat(sequenceDigits)}`}
                  />
                </div>
              </div>
        ...
        {isDeleteConfirmOpen && supplier ? (
          <div className="fixed inset-0 z-[135] flex items-center justify-center bg-black/75 px-4">
            <div className="w-full max-w-lg rounded-[2rem] border border-red-500/30 bg-zinc-950 p-6 shadow-2xl">
              <p className="text-sm font-semibold uppercase tracking-[0.24em] text-red-300/80">Delete Supplier</p>
              <h3 className="mt-3 text-2xl font-bold text-white">Delete this supplier profile?</h3>
              <p className="mt-3 text-sm leading-6 text-white/60">
                This action cannot be undone. This supplier has no related transactions, so the profile can be permanently deleted.
              </p>
              <div className="mt-5 rounded-2xl border border-white/10 bg-black/30 p-4 text-sm text-white/75">
                ...
                <div className="mt-1 text-white/50">A/C No.: {supplier.supplierAccountNo || "-"}</div>
              </div>
              <div className="mt-6 flex items-center justify-end gap-3">
                <button
                  type="button"
    ...
    <>
      ...
      <div className="rounded-3xl border border-white/20 bg-black/60 shadow-xl shadow-black/40 backdrop-blur-md">
        <div className="flex flex-col gap-3 px-5 py-5 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-white">Supplier Records</h2>
            <p className="mt-1 text-sm text-white/45">Manage supplier profiles and payable control status.</p>
          </div>
          <button type="button" onClick={() => setIsCreateOpen(true)} className="w-full rounded-xl border border-white/15 bg-black/30 px-4 py-3 text-white transition hover:bg-white/10 sm:w-auto">Add Supplier</button>
        </div>
        ...
        <table className="min-w-[980px] text-left text-sm">
          <thead className="bg-black/50 text-white/65">
            <tr>
              ...
              <th className="w-[220px] px-4 py-4">Supplier</th>
          ...
          <tbody>
            {suppliers.length > 0 ? suppliers.map((supplier, index) => (
              <tr key={supplier.id} onClick={() => handleRowClick(supplier.id)} className="cursor-pointer border-t border-white/10 align-top transition-colors hover:bg-white/[0.03]">
                <td className="px-4 py-4 text-white/55">{(currentPage - 1) * pageSize + index + 1}</td>
                <td className="break-words px-4 py-4 text-white/85">{supplier.supplierAccountNo || "-"}</td>
                <td className="px-4 py-4"><div className="break-words font-semibold text-white/90">{supplier.name}</div></td>
                <td className="break-words px-4 py-4 text-white/85">{supplier.phone || "-"}</td>
                <td className="break-words px-4 py-4 text-white/85">{supplier.email}</td>
                <td className="px-4 py-4">
                  <div className="space-y-2">
                    <span className={getCreditBadge(supplier)}>{getCreditLabel(supplier)}</span>
                    <div className="text-xs text-white/45">Outstanding {supplier.currency || "MYR"} {money(supplier.creditOutstandingAmount || 0)}</div>
                  </div>
                </td>
              ...
              <tr><td colSpan={8} className="px-4 py-10 text-center text-white/45">No suppliers found for the selected filters.</td></tr>
            )}
          </tbody>
        </table>
        </div>
