...
export function AdminCashSalesClient({
  ...
  function openEdit(transaction: CashSalesRecord) {
    if (isGeneratedFromSalesOrder(transaction)) {
      setSubmitSuccess("");
      alert("Cash Sales generated from Sales Order cannot be edited. Please cancel this DO and generate a new DO from the original SO.");
      return;
    }
  ...
  function openRevise(transaction: CashSalesRecord) {
    if (isGeneratedFromSalesOrder(transaction)) {
      setSubmitSuccess("");
      alert("Cash Sales generated from Sales Order cannot be revised. Please cancel this DO and generate a new DO from the original SO.");
      return;
    }
          ...
          <div className="mx-auto max-w-7xl rounded-[2rem] border border-white/10 bg-[#08080c] p-6 shadow-2xl">
            ...
            {hasGeneratedSalesOrderLines ? (
              <div className="mt-5 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
                This Cash Sales is generated from Sales Order. Product, qty, selling price, discount, batch/S/N, location, and footer pricing are locked. To change claim or delivery values, cancel this DO and generate again from the Sales Order.
              </div>
            ) : null}
      ...
      {isDocNoModalOpen ? (
        <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-[#08080c] p-6 shadow-2xl">
            ...
            <p className="mt-2 text-sm text-white/55">Format: CS-YYYYMMDD-0001</p>
