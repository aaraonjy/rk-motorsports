...
function buildCashSalesRevisionDocNoPreview(transaction: CashSalesRecord) {
  const baseDocNo = getBaseCashSalesDocNo(transaction.revisedFrom?.docNo || transaction.docNo);
  let maxRevision = 0;
...
export function AdminCashSalesClient({
  initialSalesOrders,
  initialCustomers,
  initialProducts,
  initialAgents,
  ...
  const docNoPreviewRequestRef = useRef(0);
  ...
  const [transactions, setTransactions] = useState<CashSalesRecord[]>([]);
  ...
  const [editTarget, setEditTarget] = useState<CashSalesRecord | null>(null);
  ...
  const systemDocNoPreview = useMemo(() => {
    if (formMode === "revise" && editTarget) {
      return buildCashSalesRevisionDocNoPreview(editTarget);
    }

    return docNo || autoGeneratedDocNoPreview;
  }, [autoGeneratedDocNoPreview, docNo, editTarget, formMode]);
  ...
  const availableSourceOrders = useMemo(() => {
    return initialSalesOrders.filter((order) => {
      ...
      return (order.lines || []).some((line) => Number(line.remainingInvoiceQty || 0) > 0);
    });
  ...
  async function loadTransactions() {
    ...
    try {
      ...
      const response = await fetch(`/api/admin/sales/cash-sales?${params.toString()}`, { cache: "no-store" });
  ...
  async function loadNextDocNo(nextDocDate = docDate) {
    const requestId = ++docNoPreviewRequestRef.current;
    try {
      const params = new URLSearchParams({ nextDocNo: "1", docDate: nextDocDate });
      const response = await fetch(`/api/admin/sales/cash-sales?${params.toString()}`, { cache: "no-store" });
      const data = await response.json();
      if (requestId !== docNoPreviewRequestRef.current) return;
  ...
  function resetForm() {
    docNoPreviewRequestRef.current += 1;
    setFormMode("create");
    setEditTarget(null);
    setActiveTab("HEADER");
  ...
  function fillFormFromTransaction(transaction: CashSalesRecord, mode: "edit" | "revise") {
    docNoPreviewRequestRef.current += 1;
    setFormMode(mode);
    ...
    setAutoGeneratedDocNoPreview(mode === "revise" ? buildCashSalesRevisionDocNoPreview(transaction) : transaction.docNo);
  ...
  function openEdit(transaction: CashSalesRecord) {
    if (isGeneratedFromSalesOrder(transaction)) {
      setSubmitSuccess("");
      alert("Cash Sales generated from Sales Order cannot be edited. Please cancel this Cash Sales and generate a new Cash Sales from the original Sales Order.");
      return;
    }
    fillFormFromTransaction(transaction, "edit");
  }

  function openRevise(transaction: CashSalesRecord) {
    if (isGeneratedFromSalesOrder(transaction)) {
      setSubmitSuccess("");
      alert("Cash Sales generated from Sales Order cannot be revised. Please cancel this Cash Sales and generate a new Cash Sales from the original Sales Order.");
      return;
    }
  ...
  function validateCashSalesForm() {
  ...
  const formValidationMessage = validateCashSalesForm();
  const canSubmitCashSales = !isSubmitting && !formValidationMessage;
  ...
  async function submitCashSales() {
    ...
    try {
      ...
      const endpoint = isUpdateMode && editTarget ? `/api/admin/sales/cash-sales/${editTarget.id}` : "/api/admin/sales/cash-sales";
      ...
      const data = await response.json();
      if (!response.ok || !data.ok) throw new Error(data.error || "Unable to save cash sales.");
      ...
      setSubmitError(error instanceof Error ? error.message : "Unable to save cash sales.");
    } finally {
  ...
  async function deletePayment(paymentId: string) {
    ...
    const confirmed = window.confirm("Delete this payment record? This will recalculate the Cash Sales outstanding balance.");
    if (!confirmed) return;
    setDeletingPaymentId(paymentId);
    setSubmitError("");
    try {
      const response = await fetch(`/api/admin/sales/cash-sales/${editTarget.id}/payments/${paymentId}`, { method: "DELETE" });
      const data = await response.json();
      if (!response.ok || !data.ok) throw new Error(data.error || "Unable to delete payment.");
      if (data.transaction) {
        setEditTarget(data.transaction as CashSalesRecord);
      } else {
  ...
  async function cancelCashSales() {
    if (!cancelTarget) return;
    try {
      const response = await fetch(`/api/admin/sales/cash-sales/${cancelTarget.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
      ...
      alert(error instanceof Error ? error.message : "Unable to cancel cash sales.");
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="mt-3 text-4xl font-bold">Cash Sales</h1>
          <p className="mt-4 max-w-3xl text-white/70">Create and manage cash sales documents. Cash Sales performs stock out immediately.</p>
        </div>
      </div>
      ...
      <div className="rounded-[2rem] border border-white/10 bg-black/45 p-5 backdrop-blur-md md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-red-400/80">Cash Sales</p>
            <h2 className="mt-4 text-2xl font-bold">Existing Cash Sales Records</h2>
            <p className="mt-4 max-w-3xl text-sm leading-6 text-white/70">Use Cash Sales to record customer delivery and stock out.</p>
          </div>
        ...
        <div className="mt-8 rounded-2xl border border-white/10 bg-black/20 p-4">
          <div className="grid gap-4 md:grid-cols-3">
            <input className="input-rk" value={searchKeyword} onChange={(e) => setSearchKeyword(e.target.value)} placeholder="Search cash sales no / customer" />
        ...
        <div className="mt-6 overflow-x-auto">
          <table className="min-w-full divide-y divide-white/10 text-sm">
            ...
            <tbody className="divide-y divide-white/10">
              {isLoading ? (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-white/50">Loading cash sales...</td></tr>
              ) : transactions.length === 0 ? (
                <tr><td colSpan={8} className="px-4 py-8 text-center text-white/50">No cash sales found.</td></tr>
              ) : (
                    ...
                    <td className="px-4 py-4 text-right">
                      {item.status !== "CANCELLED" ? (
                        <div className="flex flex-wrap justify-end gap-2">
                          {item.status !== "COMPLETED" && !isGeneratedFromSalesOrder(item) ? (
                            <>
        ...
        <div className="fixed inset-0 z-[120] overflow-y-auto bg-black/75 px-4 py-8 backdrop-blur-sm">
          <div className="mx-auto max-w-7xl rounded-[2rem] border border-white/10 bg-[#08080c] p-6 shadow-2xl">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.24em] text-red-400/80">Cash Sales</p>
                <h2 className="mt-3 text-3xl font-bold">{formMode === "revise" ? "Revise Cash Sales" : formMode === "edit" ? "Edit Cash Sales" : "Create Cash Sales"}</h2>
            ...
            {hasGeneratedSalesOrderLines ? (
              <div className="mt-5 rounded-2xl border border-amber-500/30 bg-amber-500/10 px-4 py-3 text-sm text-amber-100">
                This Cash Sales is generated from Sales Order. Product, qty, selling price, discount, batch/S/N, location, and footer pricing are locked. To change claim or delivery values, cancel this Cash Sales and generate again from the Sales Order.
              </div>
            ) : null}
              ...
              <div className="mt-6 space-y-5">
                <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                  ...
                  <div className="xl:col-span-3">
                    ...
                    <button type="button" onClick={() => { setDocNoDraft(docNo || systemDocNoPreview); setIsDocNoModalOpen(true); }} className="input-rk flex w-full items-center justify-between gap-3 pr-6 text-left">
                ...
                <div className="rounded-[1.75rem] border border-white/10 p-5">
                  <div className="mb-5 flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-2 text-sm text-white/55">Select default or secondary delivery address for this Cash Sales.</p>
                    </div>
              ...
              <div className={`mt-6 space-y-5 ${hasGeneratedSalesOrderLines ? "rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-4 opacity-70 grayscale" : ""}`}>
                {hasGeneratedSalesOrderLines ? (
                  <div className="rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white/55">
                    Read-only generated body section. This Cash Sales was generated from Sales Order, so product, qty, selling price, discount, and line pricing details are locked.
                  </div>
                ) : null}
              ...
              <div className={`mt-6 grid gap-6 lg:grid-cols-[1fr_360px] ${hasGeneratedSalesOrderLines ? "pointer-events-none rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-4 opacity-70 grayscale" : ""}`}>
                {hasGeneratedSalesOrderLines ? (
                  <div className="lg:col-span-2 rounded-2xl border border-white/10 bg-black/30 px-4 py-3 text-sm text-white/55">
                    Read-only generated footer section. This Cash Sales was generated from Sales Order, so footer pricing details are locked.
                  </div>
                ) : null}
                ...
                <div className="h-fit rounded-[1.75rem] border border-white/10 bg-black/30 p-5 text-sm">
                  <h3 className="text-xl font-semibold text-white">Cash Sales Summary</h3>
            ...
            <div className="mt-8 flex flex-wrap justify-end gap-3 border-t border-white/10 pt-5">
              ...
              <button type="button" disabled={!canSubmitCashSales} onClick={submitCashSales} className="rounded-xl bg-red-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-red-500 disabled:opacity-60">
                {isSubmitting ? "Saving..." : formMode === "revise" ? "Save Revised Cash Sales" : formMode === "edit" ? "Update Cash Sales" : "Create Cash Sales"}
              </button>
            </div>
          </div>
        </div>
      ...
      {cancelTarget ? (
        <div className="fixed inset-0 z-[150] flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm">
          <div className="w-full max-w-lg rounded-[2rem] border border-white/10 bg-[#08080c] p-6 shadow-2xl">
            <h3 className="text-2xl font-bold">Cancel Cash Sales</h3>
            ...
            <div className="mt-6 flex justify-end gap-3">
              ...
              <button type="button" onClick={cancelCashSales} className="rounded-xl bg-red-600 px-5 py-3 text-sm font-semibold text-white">Confirm Cancel</button>
            </div>
          </div>
        </div>
      ) : null}
