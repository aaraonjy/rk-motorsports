...
function canOverrideDocNoForType(config: StockSettingsConfig) {
...
function buildAutoGeneratedDocNoPreview(value: string, transactions: Array<{ docNo?: string | null; transactionNo?: string | null }>) {
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return "Auto Generated";
  const y = date.getFullYear();
  const m = String(date.getMonth() + 1).padStart(2, "0");
  const d = String(date.getDate()).padStart(2, "0");
  const prefix = `AS-${y}${m}${d}`;
  let maxSeq = 0;
...
export function AdminStockAssemblyClient({
  ...
  const [docDate, setDocDate] = useState(formatDateInput());
  ...
  const canOverrideDocNo = canOverrideDocNoForType(stockSettings);
  const autoGeneratedDocNoPreview = useMemo(
    () => buildAutoGeneratedDocNoPreview(docDate || transactionDate, transactions),
    [docDate, transactionDate, transactions]
  );
        ...
        setStockSettings(
          normalizeStockNumberFormatConfig(data.config)
            ? {
                ...
                enableProject: data.config?.enableProject ?? false,
                enableDepartment: data.config?.enableDepartment ?? false,
          ...
          <div className="max-h-[92vh] w-full max-w-6xl overflow-y-auto rounded-[2rem] border border-white/10 bg-[#0b0b0f] p-6 shadow-2xl md:p-8">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
              <div>
                ...
                <input type="date" className="input-rk" value={docDate} onChange={(e) => { setDocDate(e.target.value); setTransactionDate(e.target.value); }} />
              </div>
