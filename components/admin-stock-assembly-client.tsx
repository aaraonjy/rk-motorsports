...
function buildAutoGeneratedDocNoPreview(value: string, transactions: Array<{ docNo?: string | null }>) {
  ...
  for (const item of transactions) {
    const docNo = String(item.docNo || "").trim().toUpperCase();
    if (!docNo.startsWith(normalizedPrefix)) continue;

    const seqText = docNo.slice(normalizedPrefix.length);
