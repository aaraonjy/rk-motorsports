...
function buildCashSalesRevisionDocNoPreview(transaction: SalesInvoiceRecord) {
...
export function AdminSalesInvoiceClient({
  ...
  const isCashSalesPage = pathname.includes("/cash-sales");
  const salesApiPath = isCashSalesPage ? "/api/admin/sales/cash-sales" : "/api/admin/sales/sales-invoice";
  const salesRoutePath = isCashSalesPage ? "/admin/sales/cash-sales" : "/admin/sales/sales-invoice";
  const salesDocumentTitle = isCashSalesPage ? "Cash Sales" : "Sales Invoice";
  const salesDocumentLabel = isCashSalesPage ? "cash sales" : "sales invoice";
  ...
  const systemDocNoPreview = useMemo(() => {
    if (formMode === "revise" && isCashSalesPage && editTarget) {
      return buildCashSalesRevisionDocNoPreview(editTarget);
    }

    if (formMode === "revise" && editTarget) {
      return buildSalesInvoiceRevisionDocNoPreview(editTarget);
    }

    return docNo || autoGeneratedDocNoPreview;
  }, [autoGeneratedDocNoPreview, docNo, editTarget, formMode, isCashSalesPage]);
  ...
  async function loadTransactions() {
    ...
    try {
      ...
      const response = await fetch(`${salesApiPath}?${params.toString()}`, { cache: "no-store" });
  ...
  function fillFormFromTransaction(transaction: SalesInvoiceRecord, mode: "edit" | "revise") {
    ...
    setAutoGeneratedDocNoPreview(mode === "revise" ? (isCashSalesPage ? buildCashSalesRevisionDocNoPreview(transaction) : buildSalesInvoiceRevisionDocNoPreview(transaction)) : transaction.docNo);
  ...
  function importPickLines() {
    ...
    setSubmitMessageType("success");
    setRecentCancelledTransaction(null);
    setSubmitSuccess(`Imported ${validLines.length} Sales Order line(s). Please review and save the ${salesDocumentTitle}.`);
  }
  ...
  async function submitDeliveryOrder() {
    ...
    try {
      ...
      const endpoint = isUpdateMode && editTarget ? `${salesApiPath}/${editTarget.id}` : salesApiPath;
      ...
      const data = await response.json();
      if (!response.ok || !data.ok) throw new Error(data.error || `Unable to save ${salesDocumentLabel}.`);

      const successMessage = formMode === "revise" ? `${salesDocumentTitle} revised successfully.` : formMode === "edit" ? `${salesDocumentTitle} updated successfully.` : `${salesDocumentTitle} created successfully.`;
      setIsCreateOpen(false);
      resetForm();
      setSubmitMessageType("success");
    setRecentCancelledTransaction(null);
      ...
      setSubmitError(error instanceof Error ? error.message : `Unable to save ${salesDocumentLabel}.`);
    } finally {
  ...
  async function cancelDeliveryOrder() {
    ...
    try {
      const response = await fetch(`${salesApiPath}/${cancelTarget.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "cancel", cancelReason }),
      });
      const data = await response.json();
      if (!response.ok || !data.ok) throw new Error(data.error || `Unable to cancel ${salesDocumentLabel}.`);
      ...
      alert(error instanceof Error ? error.message : `Unable to cancel ${salesDocumentLabel}.`);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="mt-3 text-4xl font-bold">{salesDocumentTitle}</h1>
          <p className="mt-4 max-w-3xl text-white/70">{`Create and manage ${salesDocumentLabel} documents. ${salesDocumentTitle} performs stock out immediately.`}</p>
        </div>
      </div>
      ...
      <div className="rounded-[2rem] border border-white/10 bg-black/45 p-5 backdrop-blur-md md:p-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.24em] text-red-400/80">{salesDocumentTitle}</p>
            <h2 className="mt-4 text-2xl font-bold">Existing {salesDocumentTitle} Records</h2>
            <p className="mt-4 max-w-3xl text-sm leading-6 text-white/70">{`Use ${salesDocumentTitle} to record customer delivery and stock out.`}</p>
          </div>
          <button type="button" onClick={openCreate} className="inline-flex items-center justify-center rounded-xl bg-red-500 px-5 py-3 font-semibold text-white transition hover:bg-red-400">
            Create {salesDocumentTitle}
          </button>
        </div>

        <div className="mt-8 rounded-2xl border border-white/10 bg-black/20 p-4">
          <div className="grid gap-4 md:grid-cols-3">
            <input className="input-rk" value={searchKeyword} onChange={(e) => setSearchKeyword(e.target.value)} placeholder={`Search ${salesDocumentLabel} no / customer`} />
        ...
        <div className="mt-6 overflow-x-auto">
          <table className="min-w-full divide-y divide-white/10 text-sm">
            ...
            <tbody className="divide-y divide-white/10">
              {isLoading ? (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-white/50">{`Loading ${salesDocumentLabel}...`}</td></tr>
              ) : transactions.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-white/50">{`No ${salesDocumentLabel} found.`}</td></tr>
              ) : (
                transactions.map((item) => (
                  <tr key={item.id} onClick={() => router.push(`${salesRoutePath}/${item.id}`)} className="cursor-pointer text-white/80 transition hover:bg-white/[0.04]">
        ...
        <div className="fixed inset-0 z-[120] overflow-y-auto bg-black/75 px-4 py-8 backdrop-blur-sm">
          <div className="mx-auto max-w-7xl rounded-[2rem] border border-white/10 bg-[#08080c] p-6 shadow-2xl">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <p className="text-sm font-semibold uppercase tracking-[0.24em] text-red-400/80">{salesDocumentTitle}</p>
                <h2 className="mt-3 text-3xl font-bold">{formMode === "revise" ? `Revise ${salesDocumentTitle}` : formMode === "edit" ? `Edit ${salesDocumentTitle}` : `Create ${salesDocumentTitle}`}</h2>
              ...
              <div className={`mt-6 grid gap-6 lg:grid-cols-[1fr_360px] ${hasGeneratedSalesOrderLines ? "pointer-events-none rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-4 opacity-70 grayscale" : ""}`}>
                ...
                <div className="h-fit rounded-[1.75rem] border border-white/10 bg-black/30 p-5 text-sm">
                  ...
                  <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 px-4 py-3 text-xs leading-6 text-white/60">
                    Sales Invoice does not calculate tax separately at this stage. Summary keeps the same layout as Sales Order for consistency.
                  </div>
            ...
            <div className="mt-8 flex flex-wrap justify-end gap-3 border-t border-white/10 pt-5">
              ...
              <button type="button" disabled={!canSubmitDeliveryOrder} onClick={submitDeliveryOrder} className="rounded-xl bg-red-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-red-500 disabled:opacity-60">
                {isSubmitting ? "Saving..." : formMode === "revise" ? "Save Revised Sales Invoice" : formMode === "edit" ? "Update Sales Invoice" : "Create {salesDocumentTitle}"}
              </button>
            </div>
          </div>
        </div>
      ...
      {isDocNoModalOpen ? (
        <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/70 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-[2rem] border border-white/10 bg-[#08080c] p-6 shadow-2xl">
            ...
            <input className="input-rk mt-5" value={docNoDraft} onChange={(e) => setDocNoDraft(normalizeDocNoInput(e.target.value))} placeholder={systemDocNoPreview} />
            <div className="mt-6 flex justify-end gap-3">
