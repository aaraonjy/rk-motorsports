...
function buildAutoGeneratedDocNoPreview(
  ...
  const normalizedCurrentDocNo = String(currentDocNo || "").trim().toUpperCase();
  const revisionMatch = normalizedCurrentDocNo.match(new RegExp(`^(${transactionType}-\d{8}-\d{4})(?:-(\d+))?$`));
  if (revisionMatch) {
    const baseDocNo = revisionMatch[1];
    const currentRevisionNo = revisionMatch[2] ? Number(revisionMatch[2]) : 0;
    return `${baseDocNo}-${currentRevisionNo + 1}`;
  }
...
export function AdminStockTransactionEditClient({
  ...
  const projectFeatureEnabled = Boolean(stockSettings.enableProject);
  const departmentFeatureEnabled = projectFeatureEnabled && Boolean(stockSettings.enableDepartment);
  const canOverrideDocNo = canOverrideDocNoForType(stockSettings, transactionType);
  const autoGeneratedDocNoPreview = useMemo(
    () => buildAutoGeneratedDocNoPreview(transactionType, docDate || transactionDate, initialTransaction.docNo),
    [transactionType, docDate, transactionDate, initialTransaction.docNo]
  );
  ...
  useEffect(() => {
    lines.forEach((line, index) => {
      const product = initialProducts.find((item) => item.id === line.inventoryProductId);
      const shouldFetch = !!product?.serialNumberTracking && !!line.inventoryProductId && !!(transactionType === "ST" ? line.fromLocationId : line.locationId) && (!product.batchTracking || !!line.batchNo.trim());
      if (!shouldFetch) {
  ...
  function getBalanceText(productId: string, locationId: string, batchNo?: string, product?: InventoryProductOption | null, direction: "" | AdjustmentDirectionValue = "", serialCount?: number, isSerialLoading = false) {
