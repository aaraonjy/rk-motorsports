...
export function AdminStockTransactionClient({
  ...
  useEffect(() => {
    ...
    async function loadTransactions() {
      ...
      try {
        ...
        if (isMounted) setTransactions(Array.isArray(data.transactions) ? data.transactions : []);
      } catch {
        if (isMounted) setTransactions([]);
      } finally {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const created = data.transaction as TransactionRecord;
      setTransactions((prev) => (created ? [created, ...prev] : prev));
    ...
    <>
      ...
      <div className="mt-8 rounded-[2rem] border border-white/10 bg-black/45 p-6 backdrop-blur-md md:p-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.3em] text-white/45">Recent Transactions</p>
            <h3 className="mt-3 text-2xl font-bold text-white">Latest {title} Records</h3>
          </div>
        </div>
