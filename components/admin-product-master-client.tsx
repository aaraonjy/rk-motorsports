...
export function AdminProductMasterClient({
  ...
  function addOrUpdateUomConversion() {
    ...
    const normalizedRate = normalizeConversionRate(uomRateInput);
    ...
    setUomRateInput("1");
    setUomError("");
    setUomSuccess(`Saved: 1 ${normalizedCode} = ${normalizedRate} ${normalizedBase || "PCS"}`);
  }
          ...
          <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-[2rem] border border-white/10 bg-[#0b0b0f] p-6 shadow-2xl md:p-8">
            ...
            <form onSubmit={handleSubmit} className="mt-6 space-y-5">
              ...
              <div className="grid gap-3 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-white/75 md:grid-cols-5">
                ...
                <label className="flex items-center gap-3"><input type="checkbox" checked={form.serialNumberTracking} onChange={(e) => setForm((prev) => ({ ...prev, serialNumberTracking: e.target.checked }))} /><span>S/N Tracking</span></label>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/75 p-4">
          <div className="w-full max-w-2xl rounded-[2rem] border border-white/10 bg-[#0b0b0f] p-6 shadow-2xl md:p-8">
            ...
            <div className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4">
              <p className="text-sm font-semibold text-white">Saved Multi UOM</p>
