...
const SUBTITLE = "Receive stock from supplier, with optional generation from Purchase Order.";
...
export function AdminGoodsReceivedNoteClient(props: Props) {
  ...
  const [form, setForm] = useState({
    docDate: todayInput(), docDesc: "", supplierId: "", supplierName: "", supplierAccountNo: "", contactNo: "", email: "", currency: "MYR", reference: "", remarks: "", attention: "", agentId: "", projectId: "", departmentId: "", taxCodeId: "", termsAndConditions: "", bankAccount: "", footerRemarks: "",
  });
  ...
  useEffect(() => {
    ...
    setLines((source.lines || []).map((line) => ({
      ...
      uom: line.uom || "",
      qty: formatDecimalInput((line as any).remainingQty ?? (line as any).remainingReceiveQty ?? line.qty ?? 1, qtyDecimalPlaces),
  ...
  function applySupplier(supplierId: string) {
    ...
    setForm((prev) => ({ ...prev, supplierId, supplierName: supplier?.name || "", supplierAccountNo: supplier?.supplierAccountNo || "", contactNo: supplier?.phone || "", email: supplier?.email || "", attention: supplier?.attention || "", currency: supplier?.currency || "MYR", agentId: supplier?.agentId || "" }));
  }
